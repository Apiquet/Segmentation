...
"""
FCN2 with VGG16 feature extractor
"""
...
class FCN4(tf.keras.Model):
    ...
    def __init__(self, tracker_ssd_path, ssd_weights_path=None,
        ...
        self.out_stage_1 = self.x

        for i in range(3, 6):
            self.x = self.VGG16_tilStage5.get_layer(index=i)(self.x)
        self.out_stage_2 = self.x
        ...
        self.x = tf.keras.layers.Conv2DTranspose(
            n_classes, kernel_size=(4, 4), strides=(2, 2),
            use_bias=False)(self.x)
        self.x = tf.keras.layers.Cropping2D(cropping=((2, 1), (1, 2)))(self.x)

        self.out_stage_2_resized = tf.keras.layers.Conv2D(
            n_classes, (1, 1), activation='relu',
            padding='same')(self.out_stage_2)
